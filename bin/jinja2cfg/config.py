...
@summary: This Class generates a config file from a jinja2 template
...
class Cfg(object):
    ...
    def __init__(self, CfgYAMLData, CfgTemplate, CfgOutputFile=""):
        '''
        YAML data file, Jinja2 Template file are required parameters, output
        file is optional
        '''
    ...
    def generate(self):
        '''
        Generates the config. If no output file is passed, it will store the
        ...
        if (self._CfgOutputFile != ""):
            ...
            self._Logger.info("Generating config file using " +
                              self._CfgTemplate+" template")
            self._CfgGenerated = template.render(config_data)
        self._Logger.info("Done")

    def getGeneratedCfg(self):
        '''
        If no output file is passed, this method can be used to retreive the
        generated config.
